...
-- Versão: 2.0 - Implementação Corrigida
-- =====================================================
...
-- Tabela de uso dos cupons (ajustada para usar email do cliente)
...
SELECT 
  ...
  AVG(CASE WHEN av.rating IS NOT NULL THEN av.rating ELSE NULL END) AS rating_medio,
...
LEFT JOIN avaliacoes_setores av ON i.id = av.ingresso_id
...
-- Procedure para aplicar cupom de desconto (ajustada para usar email)
...
BEGIN
  ...
  ELSE
    ...
    FROM cupons_uso 
    WHERE cupom_id = v_cupom_id AND email_cliente = p_email_cliente;
...
-- 1. Removida referência à coluna cliente_id inexistente em pedidos
-- 2. Ajustado sistema de cupons para usar email_cliente
-- 3. Corrigidas todas as referências de foreign keys
-- 4. Mantidas as funcionalidades avançadas do sistema
-- =====================================================